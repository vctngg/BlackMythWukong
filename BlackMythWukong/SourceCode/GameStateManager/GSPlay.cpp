...
void GSPlay::Update(float deltaTime)
{
	...
	if ( m_Player.getHitBox()->isAlive() ) 
	{
		...
		{
			if ( m_Player.getHitBox()->getPosition().x < screenWidth / 4 ) m_Player.getHitBox()->setPosition(screenWidth/4,m_Player.getHitBox()->getPosition().y);
			if ( m_Player.getHitBox()->getPosition().x > screenWidth * 3 / 4 ) m_Player.getHitBox()->setPosition(screenWidth *3 / 4, m_Player.getHitBox()->getPosition().y);
