...
void Smash::Init()
{
	...
	m_HitBox2 = new HitBox(sf::Vector2i(16, 16));
	m_HitBox2->Init(sf::Vector2f(240, 0));
...
void Smash::Update(float deltaTime, sf::Vector2f offset)
{
	...
	{
		m_HitBox2->move(-m_HitBox2->getVelocity() * deltaTime);
		if ( m_HitBox2->getPosition().x < m_startPoint.x - 50 ) {
